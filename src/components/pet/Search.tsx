...
export default function Search({ user }: { user: User | null }) {
	...
	return (
		...
		<Container maxWidth='md' sx={{ mt: { xs: -3, md: -4 }, position: 'relative', pb: 8 }}>
			...
			{searched && <Box sx={{ mt: 6 }}><Typography variant='h2' sx={{ mb: 3 }}>{t('text-search-results')}</Typography><PetsList petsData={petsData} user={user} /></Box>}
		</Container>
		</>
	);
}

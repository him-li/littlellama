...
export default function MyPets({ user }: { user: User | null }) {
	...
	return (
		<Box>
			<Box sx={{ bgcolor: 'primary.light', py: { xs: 5, md: 7 } }}><Container maxWidth='lg'><Typography color='primary.main' fontWeight={800}>{t('mypets-kicker')}</Typography><Typography variant='h1'>{t('mypets-welcome', { name: user.firstname ?? user.firstName })}</Typography><Typography color='text.secondary' sx={{ mt: 1 }}>{collections.owned_pets.length ? t('mypets-has-pets') : t('mypets-no-pets')}</Typography></Container></Box>
